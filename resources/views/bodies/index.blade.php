    ...
    <aside class="">
        <div class="">
            <div class="add-new mb-3">
                @hasrole('doctor')
                    <a href="{{ route('bodies.create') }}"><button class="btn btn-sm btn-primary" id="add-new-btn"><i
                                    ...
                                    <td>
                                        <div class="d-flex gap-1">
                                            @if (auth()->id() === $item->user_id)
                                                <a href="{{ route('bodies.edit', $item->id) }}">
                                                    <button type="button" class="btn btn-sm btn-secondary item-edit"
                                                        data-item-id="{{ $item->id }}">
                                                        Edit
                                                    </button>
                                                </a>
                                                <form action="{{ route('bodies.destroy', $item->id) }}" method="POST"
