...
import com.example.merosathi.adapter.BodyChangeAdapter;
...
public class BodyChangeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_body_change);
    ...
    private void fetchBodyChange() {
        ...
        Call<BodyChangeList> call = apiService.bodyChangeIndex(bearerToken);

        call.enqueue(new Callback<BodyChangeList>() {
            @SuppressLint("NotifyDataSetChanged")
            @Override
            public void onResponse(Call<BodyChangeList> call, Response<BodyChangeList> response) {
                if(response.isSuccessful()){
                    BodyChangeList bodyChange = response.body();
                    assert bodyChange != null;
                    if(bodyChange.getSuccess()){
                        dataArrayList.clear();
                        dataArrayList.addAll(bodyChange.getData());
                        initializeBodyChangeAdapter();
                        bodyChangeAdapter.notifyDataSetChanged();
                    }else{
                        Toast.makeText(BodyChangeActivity.this, "Failed to retrieve data.", Toast.LENGTH_SHORT).show();
                    }
                }else{
                }
            }
            ...
            @Override
            public void onFailure(Call<BodyChangeList> call, Throwable t) {

            }
        });
    }

    private void initializeBodyChangeAdapter() {
        bodyChangeAdapter = new BodyChangeAdapter(this, dataArrayList);
